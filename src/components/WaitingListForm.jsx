        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 6 }}>
              {CLASSES.map(c => (
                <button type="button" key={c} onClick={() => toggle('classes', c)} style={chipStyle(form.classes.includes(c))}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 6 }}>
              {SEMAINES.map(s => (
                <button type="button" key={s.id} onClick={() => toggle('semaines', s.id)} style={chipStyle(form.semaines.includes(s.id))}>
                  S{s.id} — {s.label}
                </button>
              ))}
            </div>
